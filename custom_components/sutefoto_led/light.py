...
class SuteFotoLight(LightEntity):
    ...
    async def async_turn_on(self, **kwargs: Any) -> None:
        if ATTR_COLOR_TEMP_KELVIN in kwargs:
            self._instance.mode = MODE_CCT
            self._instance.cct_kelvin = kwargs[ATTR_COLOR_TEMP_KELVIN]
        elif ATTR_HS_COLOR in kwargs:
            self._instance.mode = MODE_HSI
            hue, sat = kwargs[ATTR_HS_COLOR]
            self._instance.hue = round(hue / 360 * 255)
            self._instance.saturation = round(sat)
